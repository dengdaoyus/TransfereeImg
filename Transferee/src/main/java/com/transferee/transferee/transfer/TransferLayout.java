package com.transferee.transferee.transfer;
...
import com.transferee.transferee.style.IIndexIndicator;
import com.transferee.transferee.view.image.PhotoViewPager;
...
public class TransferLayout extends FrameLayout {
    ...
    private PhotoViewPager transViewPager;
    ...
    private void createTransferAdapter() {
    ...
    private void createViewPagerAdapter() {
        transViewPager = new PhotoViewPager(context);
     ...
     */
    public void show() {
        createTransferViewPager();//创建viewpager 创 创建
     ...
     */
    public TransferState getTransferState(int position) {
        ...
        if (!transConfig.isThumbnailEmpty()) { // 客户端指定了缩略图路径集合
            ...
            if (transConfig.getImageLoader().isLoaded(url)) {
                transferState = new LocalThumState(this);//高清图图片已经加载过了，使用高清图作为缩略图。
            } else {
                transferState = new EmptyThumState(this);// 高清图尚未加载，使用原 ImageView 中显示的图片作为缩略图。
            }
        }
     ...
     */
    public void bindOnOperationListener(final TransferImage imageView, final int pos) {
